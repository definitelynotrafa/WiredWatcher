...
class ToolsConfig:
    ...
    def _update_field_display(self):
        ...
        # Simple character-based scrolling (much faster)
        max_chars = 85  # Maximum visible characters
        ...
        if len(text_to_show) > max_chars:
            ...
            if cursor_pos < self.scroll_offset:
                self.scroll_offset = max(0, cursor_pos - 10)
            elif cursor_pos > self.scroll_offset + max_chars:
                self.scroll_offset = cursor_pos - max_chars + 10
            
            text_to_show = text_to_show[self.scroll_offset:self.scroll_offset + max_chars]
            cursor_pos = cursor_pos - self.scroll_offset
        else:
            self.scroll_offset = 0
        
        # Add cursor
        if 0 <= cursor_pos <= len(text_to_show):
        ...
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, 'r') as f:
                    config = json.load(f)
                    self.nmap_command = config.get('nmap', "nmap -Pn -p- -sV -T5")
                    self.wordlist_path = config.get('wordlist', "~/wordlists")
                    self.fuzzer_choice = config.get('fuzzer', "gobuster")
                    self.gobuster_command = config.get('gobuster', "gobuster dir -w wordlist.txt -x php,txt,html,js,bak,old,zip,tar,conf,json,log")
    ...
    def _set_defaults(self):
        ...
        self.nmap_command = "nmap -Pn -p- -sV -T5"
        self.wordlist_path = "~/wordlists"
        self.fuzzer_choice = "gobuster"
        self.gobuster_command = "gobuster dir -w wordlist.txt -x php,txt,html,js,bak,old,zip,tar,conf,json,log"
        self.ffuf_command = ""
        self.current_target = ""
