...
class NotesViewer:
    ...
    def show_notes(self):
        ...
        try:
            import os
            icon_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'modules', 'assets', 'icon.ico')
            if os.path.exists(icon_path):
                self.notes_window.iconbitmap(icon_path)
        except:
            pass
        
        # Dark title bar - THIS THING IS FOR WINDOWS ONLY, WHICH IS PROBABLY THE OS THAT YOU ARE NOT USING XDDDD
        try:
            self.notes_window.update()
            DWMWA_USE_IMMERSIVE_DARK_MODE = 20
            import ctypes
            hwnd = ctypes.windll.user32.GetParent(self.notes_window.winfo_id())
